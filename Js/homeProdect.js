...
function newArrivals() {
  fetch("./data/prodects.json")
    ...
    .then(data => {
      ...
      newArrivalsContainer.innerHTML = arrivals.slice(0,4).map(item => `
      <div class="new-arrivals-card">

          <figure class="prodect-img">
            <img src="${item.images[0].url}" alt="new-arrivals-1">
          </figure>
          <h3>${item.name}</h3>
          <div class="rating">
...
function topSelling() {
  fetch("./data/prodects.json")
    ...
    .then(data => {
      ...
      topSellingContainer.innerHTML = topSelling.slice(0,4).map(item => `
      <div class="new-arrivals-card">
